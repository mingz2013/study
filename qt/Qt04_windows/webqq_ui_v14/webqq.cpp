...
void webQQ::setProxy(QString hostname, int port, QString proxyUser, QString proxyPassword)
{
    httpmethod->setProxy(hostname, port, proxyUser, proxyPassword);
}

void webQQ::startPoll2Thread()
{
    poll2thread->start();
}
void webQQ::stopPoll2Thread()
{
    poll2thread->stop();
    poll2thread->exit();
    poll2thread->wait();
